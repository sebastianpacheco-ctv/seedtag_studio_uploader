...
> **ESTADO (2026-05-29): Tier 1 + Tier 2 + Tier 3 APLICADOS y verificados.**
> Resueltos: C1 (OIDC real de Google con dominio), C2 (secret fail-closed), C3, A1
> (cookies + CSRF), A2, A3, A4, A6, A8, M1, M2, M3, M4, M6, M7 y los bajos L1-L7.
> **Pendiente:** el deploy a Firebase/Cloud Run (a la espera de proyecto+billing) y
> activar OIDC en prod (setear `GOOGLE_CLIENT_ID/SECRET` + `OAUTH_REDIRECT_URI`).
> A5 (SSE busy-poll) quedó documentado pero NO cambiado: requiere mover a polling
> client-side, es un refactor mayor para otra iteración.
